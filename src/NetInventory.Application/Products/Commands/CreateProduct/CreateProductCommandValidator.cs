...
public sealed class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
{
    ...
    public CreateProductCommandValidator()
    {
        ...
        RuleFor(x => x.SKU)
            .NotEmpty().WithMessage(Messages.Val_Product_SkuRequired)
            .Must(s => !string.IsNullOrWhiteSpace(s)).WithMessage("El SKU no puede contener solo espacios.")
            .MinimumLength(2).WithMessage("El SKU debe tener al menos 2 caracteres.")
            .MaximumLength(50).WithMessage(Messages.Val_Product_SkuMaxLength)
            .Matches(@"^[A-Za-z0-9\-_]+$").WithMessage("El SKU solo puede contener letras, números, guiones (-) y guiones bajos (_).");
